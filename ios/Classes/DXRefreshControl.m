  ...
  [self.bridge.uiManager addUIBlock:^(RCTUIManager *uiManager, RCTSparseArray *viewRegistry) {
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      if (!refreshControl) {
        ...
        JHSPullToRefreshControl *refreshControl = [[headerViewClass alloc] initWithFrame:f];
        Class contentViewClass = NSClassFromString(config[@"contentViewClass"]);
        if (contentViewClass) {
          if ([refreshControl respondsToSelector:@selector(view)]) {
            refreshControl.view = [contentViewClass new];
          }
        }
